...
function showScreen(which) {
  ...
  if (which === "styled") {
    document.title = "스타일링 결과 | AWARE AI";
  }
}
...
function setPreview(selectorId, url) {
  ...
  // styled 이미지가 세팅되면 placeholder 숨김
  if (selectorId === "preview-styled") {
    const ph = el("styled-placeholder");
...
function resetAll() {
  ...
  el("result-score").textContent = "- (10점 만점)";
  ...
  // 파일 라벨 텍스트 초기화
  const nameDisplay = el("file-name-display");
  if (nameDisplay) nameDisplay.textContent = "사진 선택하기";
  ...
  styledImg.src = "";
  styledImg.classList.add("hidden");
  const styledPh = el("styled-placeholder");
  if (styledPh) styledPh.classList.remove("hidden");

  el("tip-prompt").value = "";
  el("shopping-section").innerHTML = "";

  // 문서 타이틀 복원
  document.title = "AWARE AI";

  showScreen("upload");
}

// bfcache 복원 시 폼 초기화 (개인정보 잔존 방지)
window.addEventListener("pageshow", (e) => {
  if (e.persisted) {
    resetAll();
  }
  ...
  used.forEach(opt => {
    ...
    if (state.shoppingLoading) {
      ...
      const itemsHtml = shopItems.map(item => `
        <a class="opt-shop-item" href="${item.link}" target="_blank" rel="noopener noreferrer sponsored" onclick="event.stopPropagation()">
      ...
      shopHtml = `
        <div class="opt-shop-row">
          ...
          <button class="btn primary opt-apply-btn" data-key="${opt.key}" type="button" aria-label="${opt.title} 스타일로 이미지 생성" onclick="event.stopPropagation()">이 스타일로 이미지 생성</button>
        </div>`;
    }
